...
// Global user account page (server component)
...
export default async function AccountPage() {
  ...
  if (authError) {
    console.warn("[AccountPage] auth.getUser error:", authError.message);
  }
  ...
  const { data: existingUserRow, error: userSelectError } = await supabaseAdmin
    .from("users")
    .select("id, auth_id, email, avatar_url, subscription_tier, billing_status")
    .eq("auth_id", authId)
    .maybeSingle();

  if (userSelectError) {
    console.error("[AccountPage] users select error:", userSelectError);
  ...
  if (!userRow) {
    ...
    if (userInsertError) {
      console.error("[AccountPage] Failed to create user row:", userInsertError);
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[11px] text-muted">
              Manage your profile, global subscription, and security settings.
              This is your personal account, separate from any specific program.
            </p>
          </div>
