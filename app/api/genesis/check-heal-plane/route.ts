...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = (await req.json()) as Partial<GuardianBody>;
    if (!body?.p_season_id) return jsonError(400, "p_season_id is required");
    if (!body?.p_plane_key) return jsonError(400, "p_plane_key is required");

    const { error } = await supabase.rpc("genesis_check_and_heal_plane", {
      p_season_id: body.p_season_id,
      p_plane_key: body.p_plane_key,
    });
