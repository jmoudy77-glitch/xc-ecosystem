  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        ...
        <div className="flex items-center gap-2 rounded-full border border-slate-700 bg-slate-900/70 p-1 text-xs md:text-sm">
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 md:p-5">
        {loading && (
          <div className="text-sm text-slate-300">Loading inquiries…</div>
        )}

        {!loading && error && (
          <div className="rounded-md border border-red-500/60 bg-red-950/40 px-4 py-3 text-sm text-red-200">
            {error}
          </div>
        )}
        ...
        {!loading && !error && !hasInquiries && (
          <div className="text-sm text-slate-300">
            <p className="font-medium text-slate-100">No inquiries found.</p>
            <p className="mt-1">
              ...
              <tbody>
                {inquiries.map((inq) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 font-mono text-xs text-slate-200 md:text-sm">
                      {inq.athlete_id}
                    </td>
                    <td className="px-3 py-2 text-xs text-slate-200 md:text-sm">
                      {inq.grad_year ?? "—"}
                    </td>
                    <td className="px-3 py-2 text-xs text-slate-200 md:text-sm">
                      {inq.primary_event ?? "—"}
                    ...
                    <td className="px-3 py-2 text-xs text-slate-200 md:text-sm">
                      <div className="flex flex-wrap gap-1">
                        <button
                          ...
                          onClick={() =>
                            updateInquiryStatus(inq.id, "watching")
                          }
                          className="rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-100 hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                          Watching
                        </button>
                        <button
                          ...
                          className="rounded border border-emerald-500/60 bg-emerald-500/15 px-2 py-1 text-xs text-emerald-200 hover:bg-emerald-500/25 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                          Convert to Recruit
                        </button>
                        <button
                          ...
                          className="rounded border border-red-500/60 bg-red-500/10 px-2 py-1 text-xs text-red-200 hover:bg-red-500/20 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                          Close
                        </button>
                      </div>
