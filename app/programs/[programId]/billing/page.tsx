...
export default function ProgramBillingPage() {
  ...
  if (!programId) {
    return (
      <div className="space-y-4">
        <section className="rounded-xl border border-red-500/40 bg-red-950/60 p-4 text-xs text-red-100">
          <p className="font-semibold">Program Billing</p>
          <p className="mt-1 text-[11px] text-red-200">
            Program ID could not be resolved from the route. Try navigating here
            from the program dashboard again.
          </p>
        </section>
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <header className="rounded-xl border border-subtle bg-brand-soft p-5">
      ...
      {loading && (
        <div className="rounded-xl border border-subtle bg-surface/60 px-4 py-3 text-[11px] text-muted">
          Loading billing details…
        </div>
      )}

      {errorMsg && !loading && (
        <div className="rounded-xl border border-red-500/40 bg-red-950/60 px-4 py-3 text-xs text-red-100">
          <p className="font-semibold">Unable to manage billing</p>
          <p className="mt-1 text-[11px] text-red-200">{errorMsg}</p>
        </div>
      )}
        ...
        <div className="grid gap-4 md:grid-cols-[1.5fr,1fr]">
          ...
          <section className="rounded-xl border border-subtle bg-surface p-5 shadow-sm">
            ...
            {subscription ? (
              ...
              <div className="mt-3 space-y-2">
                ...
                <p className="text-[11px] text-muted">
                  ...
                  Choose a plan on the right and continue to Stripe checkout to
                  activate one.
                </p>
              </div>
            )}
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button
                ...
                className="inline-flex items-center rounded-full bg-brand px-4 py-1.5 text-xs font-semibold text-slate-950 hover:bg-brand-soft disabled:opacity-60"
              >
                {checkoutLoading
              ...
              <button
                ...
                className="inline-flex items-center rounded-full border border-subtle px-4 py-1.5 text-xs font-medium text-slate-100 hover:bg-surface/80 disabled:opacity-60"
              >
                {portalLoading
                  ? "Opening portal…"
          ...
          <aside className="space-y-4">
            <section className="rounded-xl border border-subtle bg-surface p-4">
              ...
              <p className="mt-1 text-[11px] text-muted">
                Select the plan you want to manage or upgrade to. The button on
                the left will send you to Stripe checkout or your existing
                subscription.
              </p>
                    ...
                    <button
                      ...
                      className={`w-full rounded-lg border px-3 py-2 text-left text-[11px] transition ${
                        isSelected
                          ...
                          : "border-subtle bg-surface/60 hover:border-brand-soft"
                      }`}
                    >
