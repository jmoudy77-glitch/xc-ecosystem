-- Kernel v1 — AI Causal Ledger Binding (S4)
-- Constitutional pattern: canonical_events -> ai_causal_ledger

drop function if exists public.kernel_emit_ai_output(
...
begin
  if p_tier < 0 or p_tier > 3 then
    raise exception 'kernel_emit_ai_output: invalid tier % (expected 0..3)', p_tier using errcode = '22023';
  end if;

  if p_confidence is not null and (p_confidence < 0 or p_confidence > 1) then
    raise exception 'kernel_emit_ai_output: invalid confidence % (expected 0..1 or null)', p_confidence using errcode = '22023';
  end if;
...
revoke all on function public.kernel_emit_ai_output(
  uuid, text, text, integer, text, uuid, uuid, jsonb, numeric, jsonb, jsonb, text, jsonb, jsonb
) from public;
