-- supabase/migrations/20260109120000_roster_planning_normalize_team_sport.sql

create or replace function public.rpc_roster_planning_normalize_team_sport_v1(p_value text)
returns text
language sql
immutable
as $$
  select case
    ...
    when lower(p_value) in ('xc', 'cross', 'cross-country', 'cross country', 'crosscountry') then 'xc'
    when lower(p_value) like '%cross%' then 'xc'
    when lower(p_value) in ('tf', 'track', 'track&field', 'track & field', 'track and field') then 'tf'
    when lower(p_value) like '%track%' then 'tf'
    else lower(p_value)
  end
$$;
...
declare
  ...
  v_sport text;
begin
  v_sport := public.rpc_roster_planning_normalize_team_sport_v1(p_sport);
  if v_sport not in ('xc','tf') then
...
begin
  ...
  v_team_sport := public.rpc_roster_planning_normalize_team_sport_v1(v_team_sport);
...
grant execute on function public.rpc_roster_planning_normalize_team_sport_v1(text) to authenticated;
